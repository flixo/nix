...
{ config, pkgs, ... }: {
  home = {
    ...
    sessionPath = [ "$HOME/bin" "$DOTFILES/bin" "$HOME/go/bin" "$HOME/.local/bin" ];
  ...
  programs = {
    ...
    git = {
      enable = true;
      userName = "Jens Nömtak";
      userEmail = "jens@flixo.se";
      lfs.enable = true;
      #diff-so-fancy.enable = true;
      # difftastic.enable = true;
      includes = [{
        condition = "hasconfig:remote.*.url:git@gitlab.com:readly-ab/**";
        contents = { user = { email = "jens.nomtak@readly.com"; }; };
      }];
      ignores =
